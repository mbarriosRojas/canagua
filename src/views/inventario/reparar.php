...
<head>
    ...
    <title>Registrar reparación - <?php echo Config::getAppName(); ?></title>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 main-content">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2"><i class="fas fa-wrench me-2"></i>Registrar reparación</h1>
                    <a href="<?php echo BASE_URL; ?>/inventario" class="btn btn-outline-secondary"><i class="fas fa-arrow-left me-1"></i> Volver</a>
                </div>
                <?php
                $itemRef = trim(($item['cod_equipo'] ?? '') . (isset($item['nombre']) && $item['nombre'] !== '' ? ' — ' . $item['nombre'] : ''));
                ?>
                <?php if ($itemRef !== ''): ?>
                <p class="text-muted mb-3">Equipo: <strong><?php echo htmlspecialchars($itemRef); ?></strong></p>
                <?php endif; ?>
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="<?php echo BASE_URL; ?>/inventario/reparar/<?php echo (int) ($item['id_inventario'] ?? 0); ?>">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label for="fecha" class="form-label">Fecha *</label>
                                    <input type="date" class="form-control" id="fecha" name="fecha" value="<?php echo htmlspecialchars($_POST['fecha'] ?? date('Y-m-d')); ?>" required>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-12 mb-3">
                                    <label for="motivo" class="form-label">Motivo *</label>
                                    <textarea class="form-control" id="motivo" name="motivo" rows="4" required><?php echo htmlspecialchars($_POST['motivo'] ?? ''); ?></textarea>
                                </div>
                            </div>
                            <hr>
